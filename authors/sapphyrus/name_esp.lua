...
local enabled_reference = ui.new_combobox("VISUALS", "Player ESP", "Name", {"Off", "Normal", "Bold"})
...
local function on_paint(ctx)
	...
	local enabled = ui_get(enabled_reference)
	if enabled ~= "Bold" then
		return
	end

	local r, g, b, a = ui_get(color_reference)
	local enemies = get_dormant_players(true)
		...
		if player ~= spectator_target then
			...
			if x1 ~= nil then
				...
				if x_center ~= nil then
					client_draw_text(ctx, x_center, y1-6, r, g, b, a*a_multiplier, "cb", 0, name)
				end
			end
		end
	end
